...
class M_ajax extends CI_Model
{
    ...
    function collabMemberId($collab_id)
    {
        $query = $this->db->query('SELECT COM_ID FROM `collab_member` WHERE COLLAB_ID = ' . $collab_id . ' AND COLMEM_STATUS = 1');
    ...
    // function get_your_new_chat($id, $com_id)
    // {

    //     $message = $this->db->query('SELECT u.USER_ID, u.USERNAME, u.USER_IMAGE, c.CHAT_ID, c.TIME, c.MESSAGE, c.COLLAB_ID FROM user u JOIN chat c on u.USER_ID = c.USER_ID WHERE c.CHAT_ID =' . $id . ' AND c.RECIPIENT_COM =' . $com_id);
    ...
    function getComMember($user_id, $com_id)
    {
        $query = $this->db->query('SELECT MEMBER_ID FROM community_member WHERE USER_ID =' . $user_id . ' AND COM_ID =' . $com_id)->row_array();
    ...
    {
        $query = $this->db->query('SELECT * FROM gallery WHERE GALLERY_ID =' . $id);
        return $query->row_array();
    }

    function getPhoto($id)
    {
        $query = $this->db->query('SELECT * FROM images WHERE IMAGE_ID =' . $id);
        return $query->row_array();
    }
    ...
    function get_last_chat($id, $user_id, $member_id)
    {
        ...
        $unseen_message = $this->db->query('SELECT c.COLLAB_ID FROM chat c JOIN community_member m on m.MEMBER_ID = c.RECIPIENT_ID WHERE c.COLLAB_ID = ' . $id . ' AND c.RECIPIENT_ID = ' . $member_id . ' AND c.READ_STATUS = 0');
    ...
    function get_unseen_message($id, $member_id)
    {
    ...
    public function loadMoreUserPost($id, $limit)
    {
        ...
        $str_query = $str_query . ') ORDER BY `UP_DATE` DESC LIMIT ' . $limit;
        return $this->db->query($str_query)->result();
    }

    public function loadMoreComPost($id, $limit)
    {
